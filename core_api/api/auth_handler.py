...
def lambda_handler(event, context):  # noqa: C901
    ...
    try:
        ...
        user_policies = iam_client.list_attached_user_policies(UserName=user_arn.split(":")[5].split("/")[1])
                    ...
                    if "Action" in statement:
                        ...
                        if "execute-api:Invoke" in actions:
                            ...
                            for resource in resources if isinstance(resources, list) else [resources]:
                                if isinstance(resource, str) and resource.endswith(f":{CORE_AUTOMATION_ADMIN_ROLE}"):
                                    has_read_role = True
                                    has_write_role = True

                                if isinstance(resource, str) and resource.endswith(f":{CORE_AUTOMATION_API_READ_ROLE}"):
                                    has_read_role = True

                                if isinstance(resource, str) and resource.endswith(f":{CORE_AUTOMATION_API_WRITE_ROLE}"):
                                    has_write_role = True
                                    has_read_role = True
